...
public:
   ...
   bool operator>( const XY& p ) const { return p < *this && p != *this; }
...
struct XYiterator
{
   bool operator!=( int ) const { return p.y < size.y; }
   XYiterator& operator++() { p.x++; if ( p.x >= size.x ) { p.x = 0; p.y++; } return *this; }
   const XY& operator*() const { return p; }

   XY p;
   XY size;
};
static XYiterator begin( const XY& size ) { return XYiterator{ XY( 0,0 ), size }; }
static int end( const XY& size ) { return 0; }
...
public:
   ...
   XYf operator/( double m ) const { return XYf( x / m, y / m ); }
   XYf mul( const XYf& p ) const { return XYf( x * p.x, y * p.y ); }

   XYf& operator+=( const XYf& p ) { return *this = *this + p; }
   XYf& operator-=( const XYf& p ) { return *this = *this - p; }

   double operator*( const XYf& p ) const { return x * p.x + y * p.y; }
   double operator^( const XYf& p ) const { return x * p.y - y * p.x; }
...
typedef boost::multiprecision::cpp_bin_float_oct myFloat;
class XYm
{
public:
   XYm() { x = y = 0; }
   XYm( const XY& p ) { x = p.x; y = p.y; }
   XYm( const myFloat& xx, const myFloat& yy ) { x = xx; y = yy; }

   XYm operator+( const XYm& p ) const { return XYm( x + p.x, y + p.y ); }
   XYm operator-( const XYm& p ) const { return XYm( x - p.x, y - p.y ); }
   XYm operator-() const { return XYm() - *this; }
   XYm operator*( myFloat m ) const { return XYm( x * m, y * m ); }
   XYm operator/( myFloat m ) const { return XYm( x / m, y / m ); }
   XYm mul( const XYm& p ) const { return XYm( x * p.x, y * p.y ); }

   XYm& operator+=( const XYm& p ) { return *this = *this + p; }
   XYm& operator-=( const XYm& p ) { return *this = *this - p; }

   myFloat operator*( const XYm& p ) const { return x * p.x + y * p.y; }
   myFloat operator^( const XYm& p ) const { return x * p.y - y * p.x; }

   myFloat len2() const { return x * x + y * y; }
   myFloat len() const;
   myFloat dist2( const XYm& p ) const { return (*this - p).len2(); }
   myFloat dist( const XYm& p ) const { return sqrt( dist2( p ) ); }

   XYm norm() const { return *this / len(); }
   XYm rot90() const { return XYm( y, -x ); }

   XYm rotated( myFloat a ) const { return XYm( x * cos( a ) - y * sin( a ), x * sin( a ) + y * cos( a ) ); }

   myFloat closeTo( const XYm& p ) const { return fabs( x - p.x ) + fabs( y - p.y ) < 1e-12; }

   XYf toXYf() const { return XYf( x.convert_to<double>(), y.convert_to<double>() ); }
   //	string str() { return "(" + x.str() + "," + y.str() + ")"; }	
